...
# from .modules.fetch_paper import
...
from .modules.latent_knowledge_analysis import get_2d_projection, get_cosine_list, get_phrase_connections, \
    get_analogy_list,get_vocablist
from .modules.citation_analysis import citation_count_query
from .modules.summarization import summarize_articles
import datetime
from .modules.word_filter import word_filter
from . modules.fetch_paper import read_from_file
...
def route_init(app):
    ...
    @app.route('/ben')
    def ben():
        return render_template('dashboard.html', title='dashboard', foo=46, data="my_data")
    ...
    def db_page():
        ...
        if (requestSeed == 'true'):
            ...
            for grant in grants.keys():
                ...
                db.save_grant(keys, grants[grant]["Post"], grants[grant]["Floor"], grants[grant]["Ceiling"],
                              grants[grant]["Close"], grants[grant]["Category"])
    ...
    def article_search(query):
        ...
        #article_list = db.search_500_articles(query)
        #article_list = db.collect_citedby("title",2953641512)
        #article_list = db.search_articles(query)

        #article_list = read_from_file()


        #db.save_current_search_from_MS(article_list)
        article_list=db.get_current_search()



        return jsonify({'article_list': article_list[:50]})
    ...
    def get_vocab():
        ...
        vocab = get_vocablist(article_list,filter)
        #print(vocab)
        return jsonify({'vocab': sorted(vocab)})
        #return jsonify({'vocab': 123})
    # tsne graph
    @app.route('/tsne')
    def tsne_ajax():
        # get the article list
        ...
        small_vecs, vocab = get_2d_projection(article_list,filter)
