...
from .modules.fetch_paper import Research_paper, Searcher
...
proxies=[]
f = open("uresearcher_app/static/proxy/Webshare 100 proxies", "r")

for x in f:
    proxies.append(x.strip())
f.close()
print("we have "+str(len(proxies)) + " proxies")
...
# query the first 500 articles
def search_500_articles(query):
    print(query)


    # creat searcher object
    searcher = Searcher(proxies)
    return_articles = searcher.querry_first_500(query)

    return return_articles

# query 500 articles of each year
def search_articles(query):
    print(query)
    # creat searcher object
    searcher = Searcher(proxies)
    return_articles = searcher.collect_all_query_articles(query)

    return return_articles



# search cited by list
def collect_citedby(title, id_):
    ...
    # creat searcher object
    searcher = Searcher(proxies)
    return_articles = searcher.collect_all_citedby_articles(title, id_)
    return return_articles


def save_current_search_from_MS(articles):
    '''Saves the most recent search in the local db (for efficiency).'''
    # delete old search
    db.session.query(CurrentSearch).delete()
    # save new search
    for article in articles:
        record = CurrentSearch(title=article.title, abstract=article.abstract, fulltext=None, doi=article.id_,
                               eid=None, link=article.source, publisher=article.authors,
                               publish_date=article.year, keywords=article.keywords)
...
def save_current_search(articles):
    ...
    # save new search
    for article in articles:
        record = CurrentSearch(title=article['title'], abstract=article['abstract'], fulltext=None, doi=article['doi'],
                               eid=None, link=article['link'], publisher=article['publisher'],
...
def save_clusters(clusters):
    ...
    for cluster in clusters:
        for article in clusters[cluster]:
            record = SavedCluster(title=article['title'], abstract=article['abstract'], fulltext=None,
                                  doi=article['doi'], eid=None, link=article['link'], publisher=article['publisher'],
                                  publish_date=article['publish_date'], keywords=article['keywords'], cluster=cluster)
            db.session.add(record)
    db.session.commit()
...
def save_citation(DOI, citations, citation_dates, references, reference_dates):
    new_citation = Citations(id=DOI, citations=citations, citation_dates=citation_dates, references=references,
...
def save_bulk_citations(tuplelist):
    ...
    for val in tuplelist:
        new_citation_list.append(
            Citations(id=val[0], citations=val[1], citation_dates=val[2], references=val[3], reference_dates=val[4]))
